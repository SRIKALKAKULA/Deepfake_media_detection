...
page = st.sidebar.selectbox("Choose Type", ["Detect Image", "Detect Video", "Detect Audio", "Detect Video (Best)", "Detect Image (Best)"])
...
if page == "Detect Image":
    ...
    checkpoint = torch.load("image_model.pth", map_location=DEVICE)
...
# ---- New Page: Image Detection (Best) ----
elif page == "Detect Image (Best)":
    ...
    if image:
        ...
        if st.button("Predict"):
            ...
            with torch.no_grad():
                ...
                st.markdown(f"### Prediction: **{label}**")
                st.markdown(f"### Raw Score: {output.item():.4f}")
